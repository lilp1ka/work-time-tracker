use crate::retrieve_tokens_from_keyring;
use anyhow::Context;
...
//     }
// }
pub async fn send_logs(log_list: Vec<ActiveApp>) -> Result<Response, anyhow::Error> {

    let (access, refresh) = retrieve_tokens_from_keyring("wtt")
        .context("Couldn't retrieve JWT tokens from keyring")?;
    ...
    let response = client
        ...
        .send()
        .await
        .context("Error while sending logs")?;

    if response.status().is_success() {
        debug!("Logs successfully sent.");
        Ok(response)
    } else {
        debug!("Server returned an error: {:?}", response.status());
        anyhow::bail!("Server error while sending logs: {}", response.status());
    }
}